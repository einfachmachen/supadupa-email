...
export function collectCandidates(copies, { profiles = [] } = {}) {
  ...
  // Der Absender läuft durch dieselbe Prüfung: Profile gelten für JEDE Person,
  // nicht nur für die eigenen Adressen.
  const froms = mkAddr("from");
...
export function pickDefaults(cands, { profiles = [], ctx = {} } = {}) {
  ...
  return {
    subject: bestSubject.value,
    from: (() => {
      const f = first(cands.froms);
      return (f?.suggestion || f?.value) ?? "";
    })(),
