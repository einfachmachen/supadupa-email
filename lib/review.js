...
/**
 * Was in der Übersicht stehen bleibt: unentschiedene Gruppen und die, die auf
 * „später nochmal prüfen“ liegen. Zusammenfassen und Löschen wandern in die
 * Vormerkungen — die Übersicht zeigt so nur noch offene Arbeit.
 */
export function pendingGroups(review) {
  if (!review) return [];
  return review.groups.filter((g) => {
    const m = markOf(review, g.key);
    return m === MARKS.none || m === MARKS.later;
  });
}
