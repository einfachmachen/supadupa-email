...
import { setHeader, splitMessage, getHeader, unwrapNestedMessage } from "./rawmail.js";
import {
  listAttachmentParts,
  listParts,
  setPartFilenames,
...
import { listCidRefs } from "./htmlmail.js";

/**
 * Zugriff auf die Thunderbird-API — bewusst LAZY über einen Proxy.
 * Ein `const api = messenger` beim Modulladen bindet die API für immer an das,
 * was in diesem Moment global stand. In Tests (und in Umgebungen, die
 * `messenger` erst spät setzen) zeigt die Bindung danach ins Leere bzw. auf
 * eine veraltete Attrappe. So wird bei JEDEM Zugriff neu nachgesehen.
 */
const api = new Proxy(
  {},
  {
    get(_t, prop) {
      const real = typeof messenger !== "undefined" ? messenger : typeof browser !== "undefined" ? browser : null;
      if (!real) throw new Error("Thunderbird-API nicht verfügbar (messenger/browser fehlt).");
      return real[prop];
    },
    has(_t, prop) {
      const real = typeof messenger !== "undefined" ? messenger : typeof browser !== "undefined" ? browser : null;
...
/** Welche Nachrichten waren verpackt? Für den Hinweis in der Oberfläche. */
const unwrappedIds = new Set();
export function wasUnwrapped(messageId) {
  return unwrappedIds.has(messageId);
}
...
export async function getRawBytes(messageId) {
  ...
  // Manche Archivierer verpacken die ganze Nachricht als Text in eine neue.
  // Ausgepackt gelesen kommen HTML-Fassung und Anhänge wieder zum Vorschein.
  const un = unwrapNestedMessage(bytes);
  if (un.unwrapped) unwrappedIds.add(messageId);
  return un.bytes;
}
...
export async function loadForLightTable(messageId, { onProgress } = {}) {
  ...
  // War die Nachricht verpackt, kennt Thunderbirds eigene Struktur nur den
  // Umschlag (eine einzige text/plain-Wolke). Dann muss der Rumpf aus den
  // ausgepackten Bytes kommen, sonst fehlen HTML-Fassung und Anhänge.
  const unwrapped = wasUnwrapped(messageId);
  const body = unwrapped ? bodyFromBytes(bytes) : collectBody(full);
  const listed = await api.messages.listAttachments(messageId).catch(() => []);

  // SICHERHEITSNETZ: Ein Teil mit Content-ID gilt nur dann als eingebettetes
  // Bild, wenn das HTML ihn auch wirklich per cid: einbindet. Alles andere
  // wandert in die Anhangsliste — lieber ein Eintrag zu viel als eine Datei,
  // die stillschweigend verschwindet.
  const html = body.html || "";
  const referenced = new Set(listCidRefs(html));
  const { attachments: parts } = classifyParts(bytes, { referencedCids: referenced });
  ...
  for (let i = 0; i < parts.length; i++) {
    ...
    attachments.push({
      ...
      // Ein Anhang, der zusätzlich im Text eingebunden ist (Outlook-Muster)
      inlineToo: Boolean(p.contentId && referenced.has(p.contentId)),
...
/**
 * Inline-Bilder (cid:) einer Nachricht als data:-URLs, für die formatierte
 * Anzeige. Nur Bilder, und nur bis zu einer vernünftigen Größe — der Rest
 * bleibt ein Platzhalter, statt die Oberfläche vollzumüllen.
 * @returns {Map<string,string>} cid → data-URL
 */
export function inlineImages(loaded, { maxBytes = 4 * 1024 * 1024 } = {}) {
  const out = new Map();
  for (const p of listParts(loaded.bytes)) {
    if (!p.contentId || !p.contentType?.startsWith("image/")) continue;
    const data = decodeBody(extractPart(loaded.bytes, p));
    if (!data.length || data.length > maxBytes) continue;
    let bin = "";
    for (const b of data) bin += String.fromCharCode(b);
    const b64 = typeof btoa === "function" ? btoa(bin) : Buffer.from(bin, "binary").toString("base64");
    out.set(p.contentId, `data:${p.contentType};base64,${b64}`);
  }
  return out;
}

/**
 * Dieselben Bilder, aber mit Metadaten — Grundlage für den Vorrat über alle
 * Kopien hinweg (`lib/inlineparts.js`).
 * @returns {Array<{cid,contentType,dataUrl,size,filename}>}
 */
export function inlineImageList(loaded, { maxBytes = 4 * 1024 * 1024 } = {}) {
  const out = [];
  for (const p of listParts(loaded.bytes)) {
    if (!p.contentId || !p.contentType?.startsWith("image/")) continue;
    const data = decodeBody(extractPart(loaded.bytes, p));
    ...
    out.push({
      ...
      filename: p.filename || "",
      size: data.length,
      dataUrl: `data:${p.contentType};base64,${b64}`,
    });
  }
  return out;
}
...
/**
 * Rumpf direkt aus den Roh-Bytes lesen — für ausgepackte Nachrichten, deren
 * Struktur Thunderbird selbst nicht kennt. Nur die Text-Teile, die nicht als
 * Anhang deklariert sind.
 */
export function bodyFromBytes(bytes) {
  const acc = { plain: "", html: "" };
  for (const p of listParts(bytes)) {
    const type = (p.contentType || "").toLowerCase();
    if (!type.startsWith("text/")) continue;
    if ((p.dispositionKind || "") === "attachment") continue;
    const text = new TextDecoder(charsetOf(p.contentTypeRaw)).decode(
      decodeBody(extractPart(bytes, p))
    );
    if (type.includes("html")) acc.html += text;
    else acc.plain += text;
  }
  return acc;
}

/** Zeichensatz aus einem Content-Type — mit utf-8 als Rückfall. */
function charsetOf(contentTypeRaw) {
  const m = /charset\s*=\s*"?([\w-]+)"?/i.exec(String(contentTypeRaw || ""));
