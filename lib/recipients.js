...
//     preferredEmail?: "die richtige Adresse, wenn die übrigen veraltet sind" }
...
export function checkRecipient(rec, profiles = []) {
  ...
  // Adresse selbst falsch: Das Profil kennt eine bevorzugte Adresse, die Mail
  // trägt aber eine alte oder eine Server-Ersatzkennung (Exchange/M365).
  const target = normalizeEmail(profile?.preferredEmail);
  if (email && target && target !== normalizeEmail(email)) {
    add("error", "old-address", `„${email}“ ist nicht mehr die richtige Adresse — richtig ist ${profile.preferredEmail}.`);
    suggestion = {
      name: suggestion?.name || profile.preferredName || name,
