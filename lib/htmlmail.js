...
const COMMENT = /<!--[\s\S]*?-->/g;
import { RTF_LEFTOVERS } from "./textclean.js";
...
export function stripWordCruft(html) {
  ...
  // Stehengebliebene RTF-Befehle („\sb280Sehr geehrter …“) — die stammen vom
  // Absender, stehen aber mitten im Fließtext und stören beim Lesen.
  s = s.replace(RTF_LEFTOVERS, "");
  return s;
}
...
/** Alle cid:-Verweise im HTML, in Reihenfolge des Auftretens. */
export function listCidRefs(html) {
  const out = [];
  const re = /<img\b[^>]*?\bsrc\s*=\s*("|')cid:([^"']+)\1/gi;
  let m;
...
/**
 * Der Namensteil einer Content-ID: `image001.png@01D71B1D.EEA0B4F0` →
 * `image001.png`. Outlook vergibt beim Weiterleiten neue IDs, behält aber den
 * Namensteil — darüber findet sich dasselbe Bild in einer anderen Kopie wieder.
 */
export function cidStem(cid) {
  return String(cid || "").split("@")[0].trim().toLowerCase();
}

/** cid:-Verweise auf mitgelieferte Bilder umbiegen; Rest neutralisieren. */
export function resolveImages(html, images = new Map(), { allowRemote = false, stems = new Map() } = {}) {
  let blockedRemote = 0;
  let missingCid = 0;
  const filled = [];
  const s = String(html || "").replace(
    ...
    (full, head, q, url) => {
      ...
      if (/^cid:/i.test(u)) {
        ...
        // Zweite Chance: dasselbe Bild unter anderer Content-ID in einer
        // Geschwister-Kopie (Outlook vergibt beim Weiterleiten neue IDs).
        const byStem = stems.get(cidStem(cid));
        if (byStem) {
          filled.push({ cid, via: "stem" });
          return `${head}${q}${byStem}${q} data-filled-from="${cidStem(cid)}"`;
        }
        missingCid++;
  ...
  return { html: s, blockedRemote, missingCid, filled };
}
...
img[data-missing-cid]::after { content: "Eingebettetes Bild fehlt in allen Kopien"; }
...
export function buildReaderDocument(html, opts = {}) {
  const { images = new Map(), stems = new Map(), showHistory = false, allowRemote = false } = opts;
  const cleaned = stripWordCruft(bodyInner(html));
  const { main, history, blocks } = splitHistory(cleaned);

  const mainPart = resolveImages(main, images, { allowRemote, stems });
  const histPart = history
    ? resolveImages(history, images, { allowRemote, stems })
    : { html: "", blockedRemote: 0, missingCid: 0, filled: [] };
  ...
  // Externe Bilder nur, wenn ausdrücklich erlaubt — sonst melden sich
  // Zählpixel beim Absender, sobald man die Mail nur ansieht.
  const csp =
    "default-src 'none'; img-src data: blob: cid:" +
    (allowRemote ? " https:" : "") +
  ...
  return {
    ...
    filled: [...(mainPart.filled || []), ...(histPart.filled || [])],
    cidRefs: listCidRefs(cleaned),
