...
export function listParts(bytes) {
  ...
  const walk = (start, end, depth) => {
    ...
    const dispositionKind = (disp.split(";")[0] || "").trim().toLowerCase();
    ...
    parts.push({
      ...
      headerEnd: hEnd,
      bodyEnd: end,
      // WICHTIG: Ein ausdrückliches `Content-Disposition: attachment` schlägt
      // alles. Outlook hängt echten Anhängen in multipart/related eine
      // Content-ID an — eine frühere Fassung hielt sie deshalb für
      // Inline-Bilder und ließ sie komplett verschwinden.
      isAttachment:
        dispositionKind === "attachment" ||
        (!cid &&
          ((!!filename && dispositionKind !== "inline") ||
            (!!filename && !mime.startsWith("text/")))),
      // Kandidat für ein eingebettetes Bild — ob es wirklich eines ist,
      // entscheidet erst der Abgleich mit den cid:-Verweisen im HTML.
      isInlineImage:
        !!cid && mime.startsWith("image/") && dispositionKind !== "attachment",
      dispositionKind,
    });
...
/**
 * Teilt die MIME-Teile in Anhänge und eingebettete Bilder auf — mit Blick auf
 * die cid:-Verweise, die im HTML wirklich vorkommen.
 *
 * Leitsatz: **Nichts darf lautlos verschwinden.** Ein Teil mit Content-ID,
 * den das HTML gar nicht einbindet, ist kein „Textbestandteil", sondern ein
 * Anhang — sonst wäre er nirgends mehr sichtbar.
 *
 * @param {Uint8Array} bytes
 * @param {{referencedCids?:Set<string>}} opts
 * @returns {{all:Array, attachments:Array, inline:Array}}
 */
export function classifyParts(bytes, { referencedCids = new Set() } = {}) {
  const all = listParts(bytes);
  const structural = (p) =>
    p.contentType.startsWith("multipart/") || p.contentType.startsWith("text/");
  const attachments = all.filter(
    (p) =>
      p.isAttachment ||
      (p.contentId &&
        !referencedCids.has(p.contentId) &&
        p.dispositionKind !== "inline" &&
        !structural(p))
  );
  const inline = all.filter((p) => p.isInlineImage && referencedCids.has(p.contentId));
  return { all, attachments, inline };
}
