...
import { comparable, clean } from "./textclean.js";
...
/** Reihenfolge vom sichersten zum großzügigsten Schärfegrad. */
export const MODE_ORDER = [MODES.strict, MODES.normal, MODES.lose];
...
/** Der nächst großzügigere Schärfegrad — oder null, wenn schon am Ende. */
export function looserMode(mode) {
  const i = MODE_ORDER.indexOf(mode);
  return i >= 0 && i < MODE_ORDER.length - 1 ? MODE_ORDER[i + 1] : null;
}

/**
 * Woran wurde diese Gruppe erkannt? Steht im Schlüssel, den `fingerprint`
 * gebaut hat — und gehört sichtbar an die Gruppe, damit man einer Zusammen-
 * fassung ansieht, wie sicher der Fund ist.
 */
export function matchExplanation(key) {
  const k = String(key || "");
  if (k.startsWith("mid:")) {
    return { level: MODES.strict, text: "gleiche Message-ID — dieselbe Nachricht" };
  }
  if (k.startsWith("norm:")) {
    return { level: MODES.normal, text: "Absender + Betreff + dieselbe Minute" };
  }
  if (k.startsWith("lose:")) {
    return { level: MODES.lose, text: "Absender + Betreff + derselbe Tag" };
  }
  return { level: MODES.normal, text: "Absender + Betreff + Zeitpunkt + Größe (keine Message-ID)" };
}
...
export function groupDuplicates(headers, { mode = MODES.normal, profiles = [] } = {}) {
  ...
  for (const [key, messages] of map) {
    ...
    groups.push({
      key,
      // Wonach gesucht wurde und woran es tatsächlich hing — im Modus
      // „normal“ können einzelne Gruppen trotzdem streng über die
      // Message-ID gefunden worden sein.
      mode,
      match: matchExplanation(key),
...
/**
 * Vergleichsform des Rumpfes. Läuft über `textclean.comparable`, d. h. Steuer-
 * und Sonderzeichen (Zero-Width, NBSP, C0/C1, Quoted-Printable-Reste,
 * Trennlinien-Rahmen) fliegen VOR dem Vergleich raus. Ohne das würden zwei
 * Kopien, die sich nur durch unsichtbaren Müll unterscheiden, nie als
...
/**
 * Fingerabdruck aus den harten Fakten statt aus dem Text: Beträge, IBAN,
 * Belegnummern, Daten — plus Größe/Typ der Anhänge. Trägt auch dann, wenn der
 * Text durch kaputte Kodierung unbrauchbar ist, und ist die zweite Chance,
 * wenn `contentKey` wegen Zeichenmülls auseinanderläuft.
 */
export function factKey(m) {
  const f = extractFacts(clean(m?.bodyText));
  const flat = [
    ...f.amounts,
    ...f.ibans,
    ...f.docNumbers,
  ...
  const atts = (m?.attachments || [])
    ...
    .sort()
    .join(",");
  return flat || atts ? `${hash(flat)}~${hash(atts)}` : "";
}
...
export function verifyGroup(group, contents) {
  ...
  for (const m of group.messages) {
    ...
    if (c) factOf.set(m.id, factKey(c));
    if (!buckets.has(key)) buckets.set(key, []);
    buckets.get(key).push(m);
  }

  // Zweite Chance über die harten Fakten: Eine Kopie, deren Text durch
  // Zeichenmüll oder kaputte Kodierung abweicht, findet über Beträge, IBAN
  // und Belegnummern trotzdem zu ihren Geschwistern zurück.
  const factIndex = new Map();
  for (const [key, list] of buckets) {
    if (key === "unverified" || list.length < 2) continue;
    const f = factOf.get(list[0].id);
    if (f) factIndex.set(f, key);
  }
  for (const [key, list] of [...buckets]) {
    if (key === "unverified" || list.length !== 1) continue;
    const f = factOf.get(list[0].id);
    const target = f && factIndex.get(f);
    if (target && target !== key) {
      buckets.get(target).push(list[0]);
      buckets.delete(key);
    }
  }
  const out = [];
