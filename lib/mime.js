...
export function splitAddressList(value) {
  ...
  // „Muster, Bernd <b@x.de>" OHNE Anführungszeichen ist formal falsch, kommt
  // aber ständig vor (Outlook-Export). Ein Stück ohne jede Adresse gehört
  // dann zum nächsten, das eine trägt — sonst zerfällt eine Person in zwei
  // Empfänger, von denen einer keine Adresse hat.
  const out = [];
  for (let i = 0; i < trimmed.length; i++) {
    const piece = trimmed[i];
    const next = trimmed[i + 1];
    const hasAddress = (v) => v.includes("@") || v.includes("<");
    if (!hasAddress(piece) && next && hasAddress(next)) {
      out.push(`${piece}, ${next}`);
      i++;
    } else {
      out.push(piece);
